            ...
            Schemas\Components\Section::make('Peran & Akses')
                ...
                ->schema([
                    Forms\Components\TextInput::make('roles')
                        ->label('Peran')
                        ->placeholder('Ketik peran (pisahkan dengan koma jika lebih dari satu)')
                        ->dehydrated(false)
                        ->afterStateHydrated(function (Forms\Components\TextInput $component, $state, ?User $record) {
                            if ($record) {
                                $component->state(implode(', ', $record->roles->pluck('name')->toArray()));
                            }
                        })
                        ->saveRelationshipsUsing(function (User $record, $state) {
                            $roles = array_filter(array_map('trim', explode(',', $state ?? '')));
                            
                            $existingRoles = \Spatie\Permission\Models\Role::whereIn('name', $roles)->pluck('name')->toArray();
                            
                            if (! auth()->user()?->isSuperAdmin()) {
                                $existingRoles = array_diff($existingRoles, [\App\Enums\UserRole::SuperAdmin->value]);
                                if ($record->isSuperAdmin()) {
                                    $existingRoles[] = \App\Enums\UserRole::SuperAdmin->value;
                                }
                            }
                            
                            $record->syncRoles($existingRoles);
                        }),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('roles')
                    ...
                    ->relationship('roles', 'name', modifyQueryUsing: function (Builder $query) {
                        if (! auth()->user()?->isSuperAdmin()) {
                            $query->where('name', '!=', \App\Enums\UserRole::SuperAdmin->value);
                        }
                    })
                    ->native(false),
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        if (! auth()->user()?->isSuperAdmin()) {
            $query->whereDoesntHave('roles', function (Builder $q) {
                $q->where('name', \App\Enums\UserRole::SuperAdmin->value);
            });
        }
